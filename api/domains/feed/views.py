...
class FeedView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if feed_type in ['all', 'movies']:
            ...
            ).filter(review_count__gt=0).order_by('-latest_review_time')[:100]
        ...
        if feed_type in ['all', 'events']:
            ...
            events = Event.objects.filter(status='FINISHED').order_by('-end_time')[:100]
            for e in events:
        ...
        # Sort combined items by sort_time descending
