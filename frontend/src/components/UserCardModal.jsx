...
function getBadge(level) {
  if (level >= 10) return { title: '奧斯卡終身成就', color: '#F59E0B', glow: 'rgba(245, 158, 11, 0.5)' };
  if (level >= 8) return { title: '去金馬作評審', color: '#8B5CF6', glow: 'rgba(139, 92, 246, 0.5)' };
  if (level >= 5) return { title: '資深影評人', color: '#3B82F6', glow: 'rgba(59, 130, 246, 0.5)' };
  if (level >= 3) return { title: '唉呦不錯呦', color: '#10B981', glow: 'rgba(16, 185, 129, 0.5)' };
  return { title: '青銅影迷', color: '#9CA3AF', glow: 'rgba(156, 163, 175, 0.5)' };
}
